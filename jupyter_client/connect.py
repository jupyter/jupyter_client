...
class ConnectionFileMixin(LoggingConfigurable):
    ...
    hb_port = Integer(0, 
            help="set the heartbeat port [default: random]").tag(config=True)
    shell_port = Integer(0, 
            help="set the shell (ROUTER) port [default: random]").tag(config=True)
    iopub_port = Integer(0,
            help="set the iopub (PUB) port [default: random]").tag(config=True)
    stdin_port = Integer(0,
            help="set the stdin (ROUTER) port [default: random]").tag(config=True)
    control_port = Integer(0,
            help="set the control (ROUTER) port [default: random]").tag(config=True)
